    ...
    snapToElement = function (propertyPanel, options) {
        ...
        setTimeout( function() {
            var offset =  AJS.Rte.Content.offset(propertyPanel.anchor),
                ppWidth = propertyPanel.panel.width(),
                overlap = ppWidth + offset.left - $(window).width() + 10,
                gapForArrowY = 7,
                gapForArrowX = 0,
            ...
            propertyPanel.panel.find(".aui-tip-parent").css({
                left: Math.min(Math.abs(offset.left - left) + 10, ppWidth - 12)
            });
        ...
        createFromButtonModel: function (type, el, buttons, options) {
            ...
            for (var i = 0, ii = buttons.length; i < ii; i++) {
                ...
                var button = buttons[i],
                    html = button.html || '<span class="icon"></span>',
                    classes = [];
